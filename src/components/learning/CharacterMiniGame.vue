...
function startBubbleRound() {
  bubbleHits.value = 0
  bubbleMisses.value = 0
  const others = distractors.value.slice(0, 5).map(c => c.character)
  const allChars = shuffle([props.character.character, ...others])
  bubbles.value = allChars.map((char, i) => ({
    id: i,
    char,
    isTarget: char === props.character.character,
    x: 15 + Math.random() * 70,
    y: 90 + Math.random() * 10,
...
function startMatchRound() {
  ...
  const options = shuffle([
    { pinyin: props.character.pinyin, isCorrect: true },
...
function selectMatch(option: { pinyin: string; isCorrect: boolean }) {
  ...
  matchSelected.value = option.pinyin
...
function startSpeedRound() {
  ...
  for (let i = 0; i < targetCount; i++) {
    rounds.push({ char: props.character.character, isTarget: true })
  }
  const dChars = distractors.value.slice(0, distractorCount).map(c => c.character)
...
function showNextSpeedChar() {
  ...
  speedTimer = setTimeout(() => {
    speedShowChar.value = false
    if (speedRounds.value[speedIndex.value].isTarget) {
      speedMisses.value++
    }
    speedIndex.value++
  ...
  <div class="character-mini-game">
    <div v-if="phase === 'intro'" class="flex flex-col items-center gap-5 py-4">
      ...
      <p class="text-sm text-gray-500 text-center px-4">
        通过三个小游戏来加深对
        <span class="text-2xl font-bold" :style="{ color }">{{ character.character }}</span>
        的印象吧！
      </p>
      <div class="grid grid-cols-3 gap-3 w-full px-2">
        ...
        <div class="flex flex-col items-center gap-1 rounded-2xl p-3" style="background-color: #FFF3D6">
          <span class="text-2xl">🔗</span>
          <span class="text-xs font-bold text-gray-500">拼音配对</span>
        </div>
    ...
    <div v-else-if="phase === 'bubble'" class="flex flex-col items-center gap-3">
      ...
      <div class="relative w-full overflow-hidden rounded-2xl" style="height: 320px; background: linear-gradient(180deg, #E8F8FF 0%, #F0F8FF 100%)">
    ...
    <div v-else-if="phase === 'match'" class="flex flex-col items-center gap-4 py-2">
      <div class="flex items-center justify-between w-full px-2">
        <div class="flex items-center gap-2">
          <span class="text-lg">🔗</span>
          <span class="text-sm font-bold text-gray-600">拼音配对</span>
        </div>
        <div class="flex items-center gap-1">
          <span
      ...
      <div class="flex flex-col items-center gap-3 w-full">
        ...
        >
          <span class="text-5xl font-bold" :style="{ color }">{{ character.character }}</span>
        </div>
        <p class="text-sm font-bold text-gray-500">选择正确的拼音</p>
          ...
          <button
            ...
            :style="{
              borderColor: matchRevealed && option.isCorrect
                ? color
                : matchRevealed && matchSelected === option.pinyin && !option.isCorrect
                  ? '#FF6B6B'
                  : matchSelected === option.pinyin
                    ? color
                    : '#E0E0E0',
        ...
        <div v-if="matchRevealed" class="text-center">
          <p class="text-base font-bold" :style="{ color: matchSelected === character.pinyin ? color : '#FF6B6B' }">
            {{ matchSelected === character.pinyin ? '✅ 正确！' : '😊 没关系，继续加油！' }}
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="phase === 'speed'" class="flex flex-col items-center gap-4 py-2">
      ...
      <div class="flex items-center gap-2 rounded-full px-4 py-1.5" :style="{ backgroundColor: color + '18' }">
        ...
        <span class="text-xl font-bold" :style="{ color }">{{ character.character }}</span>
...
.speed-char-enter-active {
  animation: speedIn 0.3s ease-out;
}
.speed-char-leave-active {
  animation: speedOut 0.2s ease-in;
}
@keyframes speedIn {
