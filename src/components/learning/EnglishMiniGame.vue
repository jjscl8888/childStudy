...
function startBubbleRound() {
  bubbleHits.value = 0
  const others = distractors.value.slice(0, 5).map(w => w.word)
  const allTexts = shuffle([props.word.word, ...others])
  bubbles.value = allTexts.map((text, i) => ({
    id: i,
    text,
    isTarget: text === props.word.word,
    x: 15 + Math.random() * 70,
    y: 90 + Math.random() * 10,
...
function startMatchRound() {
  ...
  const options = shuffle([
    { text: props.word.chinese, isCorrect: true },
    ...others.map(w => ({ text: w.chinese, isCorrect: false })),
  ])
...
function startSpeedRound() {
  ...
  for (let i = 0; i < 3; i++) {
    rounds.push({ text: props.word.word, isTarget: true })
  }
  const dItems = distractors.value.slice(0, 7).map(w => w.word)
  ...
  <div class="english-mini-game">
    <div v-if="phase === 'intro'" class="flex flex-col items-center gap-5 py-4">
      ...
      <p class="text-sm text-gray-500 text-center px-4">
        通过三个小游戏来加深对
        <span class="text-2xl font-bold" :style="{ color }">{{ word.word }}</span>
        的印象吧！
      </p>
      <div class="grid grid-cols-3 gap-3 w-full px-2">
        ...
        <div class="flex flex-col items-center gap-1 rounded-2xl p-3" style="background-color: #FFF3D6">
          <span class="text-2xl">🔗</span>
          <span class="text-xs font-bold text-gray-500">中文配对</span>
        </div>
    ...
    <div v-else-if="phase === 'bubble'" class="flex flex-col items-center gap-3">
      ...
      <div class="relative w-full overflow-hidden rounded-2xl" style="height: 320px; background: linear-gradient(180deg, #EBF5FF 0%, #D6EAFF 100%)">
        ...
        >
          <span
            class="font-bold"
            :style="{
              fontSize: Math.max(bubble.size * 0.25, 12) + 'px',
    ...
    <div v-else-if="phase === 'match'" class="flex flex-col items-center gap-4 py-2">
      <div class="flex items-center justify-between w-full px-2">
        <div class="flex items-center gap-2">
          <span class="text-lg">🔗</span>
          <span class="text-sm font-bold text-gray-600">中文配对</span>
        </div>
        <div class="flex items-center gap-1">
          <span
      ...
      <div class="flex flex-col items-center gap-3 w-full">
        ...
        >
          <span class="text-4xl font-bold" :style="{ color }">{{ word.word }}</span>
        </div>
        <p class="text-sm font-bold text-gray-500">选择对应的中文意思</p>
        ...
        <div v-if="matchRevealed" class="text-center">
          <p class="text-base font-bold" :style="{ color: matchSelected === word.chinese ? color : '#FF6B6B' }">
            {{ matchSelected === word.chinese ? '✅ 正确！' : '😊 没关系，继续加油！' }}
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="phase === 'speed'" class="flex flex-col items-center gap-4 py-2">
      ...
      <div class="flex items-center gap-2 rounded-full px-4 py-1.5" :style="{ backgroundColor: color + '18' }">
        ...
        <span class="text-xl font-bold" :style="{ color }">{{ word.word }}</span>
      ...
      <div
        ...
        style="background: linear-gradient(135deg, #EBF5FF, #D6EAFF)"
      >
        <Transition name="speed-char" mode="out-in">
          ...
          >
            <span
              class="text-5xl font-bold cursor-pointer transition-transform active:scale-90"
              :style="{
